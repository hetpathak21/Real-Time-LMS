...
const getInstructorName = (course: ICourse) => {
  if (typeof course.instructor === "object") return course.instructor.name;
  if (typeof course.teacherId === "object") return course.teacherId.name;
  if (typeof course.instructor === "string" && course.instructor.trim()) {
    return course.instructor;
  }
  return "Instructor";
};

const normalizeCourse = (course: ICourse): DashboardCourse => ({
  _id: course._id,
  title: course.title,
...
export default function CompleteAdminDashboard() {
  ...
  const platformCourses: DashboardCourse[] =
    courses?.length > 0
